...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('cont-contract-signer-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('cont-contract-signer-form-data'));
		}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('cont-contract-signer-form-data');
		}
	...
	function saveForm(param) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('cont-contract-signer-form-data-win');
				}
			} else {

...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('vehicle-maintenance-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('vehicle-maintenance-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'vehicle-maintenance-form-data-win');
			if(area==null) return;
			admin.putTempData('vehicle-maintenance-form-area', area);
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('vehicle-maintenance-form-data');
		}
		...
		if (hasData) {
			...
			//设置  报修类型 设置下拉框勾选
			fox.setSelectValue4QueryApi("#type",formData.maintenanceDict);
	...
	function saveForm(param) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('vehicle-maintenance-form-data-win');
				}
			} else {
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('vehicle-maintenance-form-data-win'); });

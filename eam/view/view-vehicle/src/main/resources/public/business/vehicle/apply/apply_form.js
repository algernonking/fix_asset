...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('vehicle-apply-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('vehicle-apply-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'vehicle-apply-form-data-win');
			if(area==null) return;
			admin.putTempData('vehicle-apply-form-area', area);
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('vehicle-apply-form-data');
		}
	...
	function saveForm(param) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('vehicle-apply-form-data-win');
				}
			} else {

...
function ListPage() {
     ...
     function renderTable() {
		...
		fox.renderTable({
			...
			cols: [[
                ...
                { field: 'status', sort: true, title: fox.translate('状态') } ,
                { field: 'categoryId', sort: true, title: fox.translate('分类ID') } ,
                { field: 'pictureId', sort: true, title: fox.translate('物品图片') } ,
                { field: 'code', sort: true, title: fox.translate('编码') } ,
                { field: 'name', sort: true, title: fox.translate('名称') } ,
                { field: 'manufacturerId', sort: true, title: fox.translate('厂商') } ,
                { field: 'brandId', sort: true, title: fox.translate('品牌') } ,
                { field: 'model', sort: true, title: fox.translate('规格型号') } ,
	...
	function bindButtonEvent() {
        ...
        $('#delete-button').click(function () {
            ...
            if(ids.length==0) {
            	layer.msg(fox.translate('请选择需要删除的')+fox.translate('资产物品')+"!");
            	return;
            }
            //调用批量删除接口
			layer.confirm(fox.translate('确定删除已选中的')+fox.translate('资产物品')+fox.translate('吗？'), function (i) {
				layer.close(i);
				layer.load(2);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				layer.confirm(fox.translate('确定删除此')+fox.translate('资产物品')+fox.translate('吗？'), function (i) {
					layer.close(i);
					layer.load(2);
	...
	function showEditForm(data) {
		...
		var title = (data && data.id) ? (fox.translate('修改')+fox.translate('资产物品')) : (fox.translate('添加')+fox.translate('资产物品'));
		admin.popupCenter({
			title: title,
			resize:true,
			offset:[top,null],

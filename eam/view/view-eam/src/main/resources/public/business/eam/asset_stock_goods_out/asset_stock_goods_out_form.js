...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-asset-stock-goods-out-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-asset-stock-goods-out-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-asset-stock-goods-out-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-asset-stock-goods-out-form-area', area);
	...
	function renderFormFields() {
		...
		laydate.render({
			elem: '#collectionDate',
			format:"yyyy-MM-dd",
			value:$('#collectionDate').val()?$('#collectionDate').val():new Date(),
			trigger:"click",
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-asset-stock-goods-out-form-data');
		}
	...
	function saveForm(param) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-asset-stock-goods-out-form-data-win');
				}
			} else {
    ...
    function bindButtonEvent() {
		...
		// 请选择公司对话框
		$("#useOwnCompanyId-button").click(function(){
			var useOwnCompanyIdDialogOptions={
				field:"useOwnCompanyId",
				formData:getFormData(),
				inputEl:$("#useOwnCompanyId"),
				buttonEl:$(this),
				single:true,
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-asset-stock-goods-out-form-data-win'); });

...
function FormPage() {
	...
	const moduleURL="/service-eam/eam-repair-category";
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-repair-category-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-repair-category-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-repair-category-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-repair-category-form-area', area);
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-repair-category-form-data');
		}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-repair-category-form-data-win');
				}
    ...
    function bindButtonEvent() {
		...
		form.on('submit(submit-button)', function (data) {
			...
			admin.request(api, data.field, function (res) {
				...
				if (res.success) {
					if (parent){
						parent.changeNodeName(data.field.id,data.field.repairName);
					}
		...
		//关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-repair-category-form-data-win',this); });

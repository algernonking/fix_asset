...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-asset-process-record-form-data-form-action');
		//如果没有修改和保存权限
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			if(area==null) return;
			admin.putTempData('eam-asset-process-record-form-area', area);
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-asset-process-record-form-data');
		}
		...
		if (hasData) {
			...
			//设置 变更人 显示复选框勾选
			if(formData["processUserId"]) {
				$("#processUserId").val(fox.dateFormat(formData["processUserId"],"yyyy-MM-dd HH:mm:ss"));
			}
			//设置 变更时间 显示复选框勾选
			if(formData["processdTime"]) {
				$("#processdTime").val(fox.dateFormat(formData["processdTime"],"yyyy-MM-dd HH:mm:ss"));
			}
	...
	function saveForm(param) {
		...
		admin.request(api, param, function (data) {
			...
			if (data.success) {
				...
				var index=admin.getTempData('eam-asset-process-record-form-data-popup-index');
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
		$("#cancel-button").click(function(){admin.finishPopupCenterById("eam-asset-process-record-form-data-win");});

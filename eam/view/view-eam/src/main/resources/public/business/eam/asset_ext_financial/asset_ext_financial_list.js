...
function ListPage() {
		...
		function renderTableInternal() {
			...
			dataTable=fox.renderTable({
				...
				cols: [[
					...
					,{ field: 'assetCode', align:"left",fixed:false,  hide:false, sort: true, title: fox.translate('财务编号') , templet: function (d) { return templet('assetCode',d.assetCode,d);}  }
					...
					,{ field: 'notes', align:"left",fixed:false,  hide:false, sort: true, title: fox.translate('财务备注') , templet: function (d) { return templet('notes',d.notes,d);}  }
	...
	function refreshTableData(sortField,sortType) {
		var value = {};
		value.financialCategoryId={ value: $("#financialCategoryId").val()};
	...
	function bindButtonEvent() {
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('资产财务数据')+"!");
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的')+fox.translate('资产财务数据')+fox.translate('吗？'), function (i) {
				top.layer.close(i);
				top.layer.load(2);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-asset-ext-financial-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				//延迟显示加载动画，避免界面闪动
				...
				admin.request(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('eam-asset-ext-financial-form-data-form-action', "edit",true);
				...
				admin.request(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					...
					layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('eam-asset-ext-financial-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						layer.msg(data.message, {icon: 1, time: 1500});
					}
				...
				top.layer.confirm(fox.translate('确定删除此')+fox.translate('资产财务数据')+fox.translate('吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-asset-ext-financial-form-data-form-action');
		var queryString="";
		...
		var title = fox.translate('资产财务数据');
		...
		var index=admin.popupCenter({
			...
			area: ["85%",height+"px"],
			type: 2,
			id:"eam-asset-ext-financial-form-data-win",
			content: '/business/eam/asset_ext_financial/asset_ext_financial_form.html' + queryString,
			finish: function () {
				refreshTableData();
			}
		});

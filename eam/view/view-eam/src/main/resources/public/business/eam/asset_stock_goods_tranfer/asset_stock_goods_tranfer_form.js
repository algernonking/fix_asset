...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-asset-stock-goods-tranfer-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-asset-stock-goods-tranfer-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-asset-stock-goods-tranfer-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-asset-stock-goods-tranfer-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("warehouseOutId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-asset-stock-goods-tranfer-form-data');
		}
		...
		if (hasData) {
			...
			//设置  调出仓库 设置下拉框勾选
			fox.setSelectValue4QueryApi("#warehouseOutId",formData.warehouseOut);
			//设置  调入仓库 设置下拉框勾选
	...
	function getFormData() {
		...
		//获取 调出仓库 下拉框的值
		data["warehouseOutId"]=fox.getSelectedValue("warehouseOutId",false);
		//获取 调入仓库 下拉框的值
	...
	function saveForm(param) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-asset-stock-goods-tranfer-form-data-win');
				}
			} else {
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-asset-stock-goods-tranfer-form-data-win'); });

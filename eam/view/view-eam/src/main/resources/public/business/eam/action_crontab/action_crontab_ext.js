...
//
layui.define(['cron','form', 'table', 'util', 'settings', 'admin', 'upload','foxnic','xmSelect','laydate','foxnicUpload','dropdown'],function () {
    ...
    var formAction=admin.getTempData('eam-action-crontab-form-data-form-action');

    var c=cron.render({
        elem: "#crontab", // 绑定元素
        value: $("#crontab").val(), // 默认值
       // run: "../static/json/run.json", // 获取最近运行时间的接口
        done: function(constr) {
            console.log('result',constr);
        },
    });

...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-repair-order-acceptance-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-repair-order-acceptance-form-data'));
		}
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("resultType",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("categoryTplId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
		...
		laydate.render({
			elem: '#finishTime',
			format:"yyyy-MM-dd",
			value:$('#finishTime').val()?$('#finishTime').val():new Date(),
			trigger:"click",
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-repair-order-acceptance-form-data');
		}
		...
		if (hasData) {
			...
			if(formData["finishTime"]) {
				$("#finishTime").val(fox.dateFormat(formData["finishTime"],"yyyy-MM-dd"));
			}


			//设置  维修结果 设置下拉框勾选
			fox.setSelectValue4QueryApi("#resultType",formData.resultTypeDict);
			//设置  实际故障 设置下拉框勾选
	...
	function getFormData() {
		...
		//获取 维修结果 下拉框的值
		data["resultType"]=fox.getSelectedValue("resultType",false);
		//获取 实际故障 下拉框的值
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-repair-order-acceptance-form-data-win');
				}
    ...
    function bindButtonEvent() {
		...
		// 请选择人员对话框
		$("#accepterId-button").click(function(){
				var accepterIdDialogOptions={
				field:"accepterId",
			...
			fox.chooseEmployee(accepterIdDialogOptions);
		});

	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-repair-order-acceptance-form-data-win',this); });

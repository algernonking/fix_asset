...
function FormPage() {
	...
	this.init=function(layui) {
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-repair-order-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-repair-order-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-repair-order-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("urgencyId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-repair-order-form-data');
		}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-repair-order-form-data-win');
				}
    ...
    function bindButtonEvent() {
		...
		// 请选择组织节点对话框
		$("#reportOrgId-button").click(function(){
			var reportOrgIdDialogOptions={
				field:"reportOrgId",
			...
			fox.chooseOrgNode(reportOrgIdDialogOptions);
		});
		// 请选择人员对话框
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-repair-order-form-data-win',this); });

...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-repair-rule-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-repair-rule-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-repair-rule-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-repair-rule-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("userId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-repair-rule-form-data');
		}
		...
		if (hasData) {
			...
			//设置  成员 设置下拉框勾选
			fox.setSelectValue4QueryApi("#userId",formData.user);
	...
	function getFormData() {
		...
		//获取 成员 下拉框的值
		data["userId"]=fox.getSelectedValue("userId",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-repair-rule-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-repair-rule-form-data-win',this); });

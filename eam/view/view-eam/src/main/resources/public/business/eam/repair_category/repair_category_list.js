...
function ListPage() {
	...
	function beforeNodeRemove(treeId, treeNode) {
		//debugger;
		layer.confirm('确定要删除['+treeNode.repairName+']菜单吗?', function(index,a,c,d) {
			layer.close(index);
...
//			}
	...
	$('#btn-add').click(function () {
		...
		admin.request(moduleURL+"/insert",{parentId:treeNode?treeNode.id:"0",repairName:"新菜单"},function(r) {
			if(r.success) {
				admin.toast().success("菜单已创建",{time:1000,position:"right-bottom"});
				//debugger
				if(treeNode==null) {
					//debugger;
					menuTree.addNodes(null,{id:r.data.id,repairName:r.data.repairName});
					return;
				}
				var isLeaf=!treeNode.isParent;
				treeNode.isParent=true;
				menuTree.updateNode(treeNode)
				menuTree.addNodes(treeNode,{id:r.data.id,repairName:r.data.repairName,parentId:r.data.parentId});
			} else {

...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-ops/workorder-type";
	...
	function bindButtonEvent() {
        ...
        function batchDelete() {
            ...
            if(ids.length==0) {
            	layer.msg(fox.translate('请选择需要删除的')+fox.translate('工单类型')+"!");
            	return;
            }
            //调用批量删除接口
			layer.confirm(fox.translate('确定删除已选中的')+fox.translate('工单类型')+fox.translate('吗？'), function (i) {
				layer.close(i);
				layer.load(2);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				layer.confirm(fox.translate('确定删除此')+fox.translate('工单类型')+fox.translate('吗？'), function (i) {
					layer.close(i);
					layer.load(2);
	...
	function showEditForm(data) {
		...
		admin.putTempData('workorder-type-form-data', data);
		var area=admin.getTempData('workorder-type-form-area');
		...
		admin.popupCenter({
			...
			type: 2,
			content: '/business/ops/workorder_type/workorder_type_form.html' + queryString,
			finish: function () {
				refreshTableData();
			}
		});

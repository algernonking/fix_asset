...
function ListPage() {
	...
	function bindButtonEvent() {
        ...
        function batchDelete() {
            ...
            if(ids.length==0) {
            	layer.msg(fox.translate('请选择需要删除的')+fox.translate('区域')+"!");
            	return;
            }
            //调用批量删除接口
			layer.confirm(fox.translate('确定删除已选中的')+fox.translate('区域')+fox.translate('吗？'), function (i) {
				layer.close(i);
				layer.load(2);
	...
	function showEditForm(data) {
		...
		var title = (data && data.id) ? (fox.translate('修改')+fox.translate('区域')) : (fox.translate('添加')+fox.translate('区域'));
		var index=admin.popupCenter({
			...
			type: 2,
			content: '/business/datacenter/area/area_form.html' + queryString,
			finish: function () {
				refreshTableData();
			}
		});
		admin.putTempData('dc-area-form-data-popup-index', index);
	};

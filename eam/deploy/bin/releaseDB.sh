...
$MYSQL_DUMP -u$DB_USER -p$DB_PWD -h$DB_HOST eam  > /tmp/db.sql
$MYSQL -u$DB_USER -p$DB_PWD -h$DB_HOST eam_demo < /tmp/db.sql
$MYSQL -u$DB_USER -p$DB_PWD -h$DB_HOST eam_demo < $cur_dir/sql/clearData.sql
$MYSQL_DUMP -u$DB_USER -p$DB_PWD -h$DB_HOST eam_demo  > /tmp/db.sql
cd /tmp/
f=eam_db_$TODAY.tar.gz
tar zcvf eam_db_$TODAY.tar.gz db.sql
echo "success /tmp/$f"
exit 0

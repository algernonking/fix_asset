...
public interface CategoryFinanceServiceProxy {
	...
	*/
	...
	Result insert(CategoryFinanceVO categoryFinanceVO);
	...
	*/
	...
	Result update(CategoryFinanceVO categoryFinanceVO);
	
	/**
	 * 更新财务分类
	*/
	@RequestMapping(CategoryFinanceServiceProxy.SAVE)
	Result save(CategoryFinanceVO categoryFinanceVO);
	
	/**
	 * 获取财务分类
	*/
	@RequestMapping(CategoryFinanceServiceProxy.GET_BY_ID)
	Result<CategoryFinance> getById(String id);

	/**
	 * 批量删除财务分类
	*/
	@RequestMapping(CategoryFinanceServiceProxy.GET_BY_IDS)
	Result<List<CategoryFinance>> getByIds(List<String> ids);
	/**
	 * 查询财务分类
	*/
	@RequestMapping(CategoryFinanceServiceProxy.QUERY_LIST)
	Result<List<CategoryFinance>> queryList(CategoryFinanceVO sample);
	
	/**
	 * 分页查询财务分类
	*/
	@RequestMapping(CategoryFinanceServiceProxy.QUERY_PAGED_LIST)
	Result<PagedList<CategoryFinance>> queryPagedList(CategoryFinanceVO sample);

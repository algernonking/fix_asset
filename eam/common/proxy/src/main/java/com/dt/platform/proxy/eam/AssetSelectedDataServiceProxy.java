...
public interface AssetSelectedDataServiceProxy {
	...
	*/
	...
	Result insert(AssetSelectedDataVO assetSelectedDataVO);
	...
	*/
	...
	Result update(AssetSelectedDataVO assetSelectedDataVO);
	
	/**
	 * 更新资产选择
	*/
	@RequestMapping(AssetSelectedDataServiceProxy.SAVE)
	Result save(AssetSelectedDataVO assetSelectedDataVO);
	
	/**
	 * 获取资产选择
	*/
	@RequestMapping(AssetSelectedDataServiceProxy.GET_BY_ID)
	Result<AssetSelectedData> getById(String id);

	/**
	 * 批量删除资产选择
	*/
	@RequestMapping(AssetSelectedDataServiceProxy.GET_BY_IDS)
	Result<List<AssetSelectedData>> getByIds(List<String> ids);
	/**
	 * 查询资产选择
	*/
	@RequestMapping(AssetSelectedDataServiceProxy.QUERY_LIST)
	Result<List<AssetSelectedData>> queryList(AssetSelectedDataVO sample);
	
	/**
	 * 分页查询资产选择
	*/
	@RequestMapping(AssetSelectedDataServiceProxy.SAVE_BY_IDS)
	Result saveById(List<String> ids,String assetSelectedCode);

	/**
	 * 分页查询资产选择
	 */
	@RequestMapping(AssetSelectedDataServiceProxy.QUERY_PAGED_LIST)
	Result<PagedList<AssetSelectedData>> queryPagedList(AssetSelectedDataVO sample);

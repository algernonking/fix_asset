...
public interface DbInstanceServiceProxy {
	...
	*/
	...
	Result insert(DbInstanceVO dbInstanceVO);
	...
	*/
	...
	Result update(DbInstanceVO dbInstanceVO);
	
	/**
	 * 更新数据库实例
	*/
	@RequestMapping(DbInstanceServiceProxy.SAVE)
	Result save(DbInstanceVO dbInstanceVO);
	
	/**
	 * 获取数据库实例
	*/
	@RequestMapping(DbInstanceServiceProxy.GET_BY_ID)
	Result<DbInstance> getById(String id);

	/**
	 * 批量删除数据库实例
	*/
	@RequestMapping(DbInstanceServiceProxy.GET_BY_IDS)
	Result<List<DbInstance>> getByIds(List<String> ids);
	/**
	 * 查询数据库实例
	*/
	@RequestMapping(DbInstanceServiceProxy.QUERY_LIST)
	Result<List<DbInstance>> queryList(DbInstanceVO sample);
	
	/**
	 * 分页查询数据库实例
	*/
	@RequestMapping(DbInstanceServiceProxy.QUERY_PAGED_LIST)
	Result<PagedList<DbInstance>> queryPagedList(DbInstanceVO sample);

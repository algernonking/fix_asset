...
public interface ResidualStrategyServiceProxy {
    ...
    Result insert(@RequestParam(name = "residualStrategyVO") ResidualStrategyVO residualStrategyVO);
    ...
    Result update(@RequestParam(name = "residualStrategyVO") ResidualStrategyVO residualStrategyVO);

    /**
     * 更新折旧策略
     */
    @RequestMapping(ResidualStrategyServiceProxy.SAVE)
    Result save(@RequestParam(name = "residualStrategyVO") ResidualStrategyVO residualStrategyVO);

    /**
     * 获取折旧策略
     */
    @RequestMapping(ResidualStrategyServiceProxy.GET_BY_ID)
    Result<ResidualStrategy> getById(@RequestParam(name = "id") String id);

    /**
     * 批量删除折旧策略
     */
    @RequestMapping(ResidualStrategyServiceProxy.GET_BY_IDS)
    Result<List<ResidualStrategy>> getByIds(@RequestParam(name = "ids") List<String> ids);

    /**
     * 查询折旧策略
     */
    @RequestMapping(ResidualStrategyServiceProxy.QUERY_LIST)
    Result<List<ResidualStrategy>> queryList(@RequestParam(name = "sample") ResidualStrategyVO sample);

    /**
     * 分页查询折旧策略
     */
    @RequestMapping(ResidualStrategyServiceProxy.QUERY_PAGED_LIST)
    Result<PagedList<ResidualStrategy>> queryPagedList(@RequestParam(name = "sample") ResidualStrategyVO sample);

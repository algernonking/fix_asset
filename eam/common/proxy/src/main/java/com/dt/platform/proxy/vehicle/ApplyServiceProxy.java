...
;
	...
	*/
	...
	Result update(ApplyVO applyVO);

	/**
	 * 更新车辆申请
	*/
	@RequestMapping(ApplyServiceProxy.SAVE)
	Result save(ApplyVO applyVO);

	/**
	 * 获取车辆申请
	*/
	@RequestMapping(ApplyServiceProxy.GET_BY_ID)
	Result<Apply> getById(String id);

	/**
	 * 获取多个车辆申请
	*/
	@RequestMapping(ApplyServiceProxy.GET_BY_IDS)
	Result<List<Apply>> getByIds(List<String> ids);
	/**
	 * 查询车辆申请
	*/
	@RequestMapping(ApplyServiceProxy.QUERY_LIST)
	Result<List<Apply>> queryList(ApplyVO sample);

	/**
	 * 分页查询车辆申请
	*/
	@RequestMapping(ApplyServiceProxy.QUERY_PAGED_LIST)
	Result<PagedList<Apply>> queryPagedList(ApplyVO sample);

...
@FeignClient(value = EAMServiceNames.OPS, contextId = HostServiceProxy.API_CONTEXT_PATH , configuration = FeignConfiguration.class)
public interface HostServiceProxy {
	...
	*/
	...
	Result update(HostVO hostVO);
	
	/**
	 * 更新主机
	*/
	@RequestMapping(HostServiceProxy.SAVE)
	Result save(HostVO hostVO);
	
	/**
	 * 获取主机
	*/
	@RequestMapping(HostServiceProxy.GET_BY_ID)
	Result<Host> getById(String id);

	/**
	 * 批量删除主机
	*/
	@RequestMapping(HostServiceProxy.GET_BY_IDS)
	Result<List<Host>> getByIds(List<String> ids);
	/**
	 * 查询主机
	*/
	@RequestMapping(HostServiceProxy.QUERY_LIST)
	Result<List<Host>> queryList(HostVO sample);
	
	/**
	 * 分页查询主机
	*/
	@RequestMapping(HostServiceProxy.QUERY_PAGED_LIST)
	Result<PagedList<Host>> queryPagedList(HostVO sample);
	
	
	/**
	 * 控制器类名
	 * */
	public static final String CONTROLLER_CLASS_NAME="com.dt.eam.ops.controller.HostController";

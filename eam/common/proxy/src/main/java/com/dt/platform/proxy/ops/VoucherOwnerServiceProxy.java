...
public interface VoucherOwnerServiceProxy {
    ...
    Result insert(@RequestParam(name = "voucherOwnerVO") VoucherOwnerVO voucherOwnerVO);
    ...
    Result update(@RequestParam(name = "voucherOwnerVO") VoucherOwnerVO voucherOwnerVO);

    /**
     * 更新所属凭证
     */
    @RequestMapping(VoucherOwnerServiceProxy.SAVE)
    Result save(@RequestParam(name = "voucherOwnerVO") VoucherOwnerVO voucherOwnerVO);

    /**
     * 获取所属凭证
     */
    @RequestMapping(VoucherOwnerServiceProxy.GET_BY_ID)
    Result<VoucherOwner> getById(@RequestParam(name = "id") String id);

    /**
     * 批量删除所属凭证
     */
    @RequestMapping(VoucherOwnerServiceProxy.GET_BY_IDS)
    Result<List<VoucherOwner>> getByIds(@RequestParam(name = "ids") List<String> ids);

    /**
     * 查询所属凭证
     */
    @RequestMapping(VoucherOwnerServiceProxy.QUERY_LIST)
    Result<List<VoucherOwner>> queryList(@RequestParam(name = "sample") VoucherOwnerVO sample);

    /**
     * 分页查询所属凭证
     */
    @RequestMapping(VoucherOwnerServiceProxy.QUERY_PAGED_LIST)
    Result<PagedList<VoucherOwner>> queryPagedList(@RequestParam(name = "sample") VoucherOwnerVO sample);

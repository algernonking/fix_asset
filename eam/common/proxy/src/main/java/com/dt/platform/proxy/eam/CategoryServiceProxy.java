...
public interface CategoryServiceProxy {
	...
	*/
	...
	Result insert(CategoryVO categoryVO);
	...
	*/
	...
	Result save(CategoryVO categoryVO);
	
	/**
	 * 获取资产分类
	*/
	@RequestMapping(CategoryServiceProxy.GET_BY_ID)
	Result<Category> getById(String id);

	/**
	 * 批量删除资产分类
	*/
	@RequestMapping(CategoryServiceProxy.GET_BY_IDS)
	Result<List<Category>> getByIds(List<String> ids);
	/**
	 * 查询资产分类
	*/
	@RequestMapping(CategoryServiceProxy.QUERY_LIST)
	Result<List<Category>> queryList(CategoryVO sample);
	
	/**
	 * 分页查询资产分类
	*/
	@RequestMapping(CategoryServiceProxy.QUERY_PAGED_LIST)
	Result<PagedList<Category>> queryPagedList(CategoryVO sample);

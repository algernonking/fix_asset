...
public interface AssetBorrowServiceProxy {
	...
	*/
	...
	Result update(AssetBorrowVO assetBorrowVO);
	
	/**
	 * 更新资产借用
	*/
	@RequestMapping(AssetBorrowServiceProxy.SAVE)
	Result save(AssetBorrowVO assetBorrowVO);
	
	/**
	 * 获取资产借用
	*/
	@RequestMapping(AssetBorrowServiceProxy.GET_BY_ID)
	Result<AssetBorrow> getById(String id);

	/**
	 * 批量删除资产借用
	*/
	@RequestMapping(AssetBorrowServiceProxy.GET_BY_IDS)
	Result<List<AssetBorrow>> getByIds(List<String> ids);
	/**
	 * 查询资产借用
	*/
	@RequestMapping(AssetBorrowServiceProxy.QUERY_LIST)
	Result<List<AssetBorrow>> queryList(AssetBorrowVO sample);
	
	/**
	 * 分页查询资产借用
	*/
	@RequestMapping(AssetBorrowServiceProxy.QUERY_PAGED_LIST)
	Result<PagedList<AssetBorrow>> queryPagedList(AssetBorrowVO sample);
